"""MetaWorld MT1 single-task experiment with SAC and reset methods.

This script runs SAC on individual MetaWorld MT10 tasks (as MT1) with various
reset methods. Unlike MT10 continual learning, this trains a single task from
scratch to evaluate single-task RL performance.

Usage:
    # Single run (smoke test)
    python -m experiments.metaworld_mt1 --task-name reach-v3 --optimizer adam --seed 0 --wandb-mode disabled

    # Full run with logging
    python -m experiments.metaworld_mt1 --task-name push-v3 --optimizer cbp --seed 0 --wandb-entity your_entity

    # With custom steps
    python -m experiments.metaworld_mt1 --task-name reach-v3 --optimizer adam --seed 0 --total-steps 100000 --wandb-mode disabled
"""
...
def evaluate(env, sac_state, key, num_episodes: int = 10):
    ...
    current_length = np.zeros(1, dtype=int)
