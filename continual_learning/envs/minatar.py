"""Official MinAtar continual learning benchmark.

Chains three MinAtar tasks: SpaceInvaders → Asterix → Seaquest.
Observations are padded to (10, 10, 10) and flattened to 1000 dims.

Uses the official MinAtar library (https://github.com/kenjyoung/MinAtar).
All three games have 6 actions and obs channels:
  space_invaders: 6ch, asterix: 4ch, seaquest: 10ch.
"""
...
class MinatarVectorEnv(VectorEnv):
    ...
    Manages num_envs parallel environments using a list of minatar.Environment
    objects stepped in a Python loop (same pattern as MetaWorldVectorEnv).
    Observations are channel-padded to (10, 10, 10) and flattened to 1000 dims.
    Done environments are auto-reset after each step.
    """
