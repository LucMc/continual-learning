from continual_learning.envs.slippery_ant_v5 import ContinualAntEnv
import argparse
import string
...
class Actor(nn.Module):
    ...
    def __call__(self, x: Array):
        ...
        actor_logstd = self.param(
            "logstd", nn.initializers.zeros, (1, self.action_shape_prod)
        )
        action_logstd = jnp.broadcast_to(
            actor_logstd, action_mean.shape
...
def parse_args():
    ...
    parser.add_argument(
        "--num-envs", type=int, default=1, help="the number of parallel environments"
    )
    parser.add_argument(
        "--num-steps",
        type=int,
    ...
    parser.add_argument(
        "--seed", type=int, default=1, help="seed for reproducible benchmarks"
    )
    parser.add_argument(
        "--exp-name",
        type=str,
    ...
    parser.add_argument(
        "--env-id", type=str, default="Pusher-v4", help="id of the environment"
    )
    parser.add_argument(
        "--capture-video",
        type=bool,
        default=False,
        help="whether to save video of agent gameplay",
    )
    parser.add_argument(
        "--track", type=bool, default=False, help="whether to track project with W&B"
    )
    parser.add_argument(
        "--wandb-project-name",
        type=str,
...
def linear_schedule(count):
    frac = (
        1.0 - (count // (args.num_minibatches * args.update_epochs)) / args.num_updates
    )
...
def get_action_and_value(
    ...
    action_mean, action_logstd = agent_state.actor_fn(
...
def get_action_and_value2(
    agent_state: AgentState, params: AgentParams, obs: ndarray, action: ndarray
):
...
@jit
def compute_gae(
    agent_state: AgentState, next_obs: ndarray, next_done: ndarray, storage: Storage
):
...
def update_ppo(agent_state: AgentState, storage: Storage, key: random.PRNGKey):
...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(args.env_id, i, args.capture_video, args.exp_name)
    ...
    actor = Actor(
        action_shape_prod=np.array(envs.single_action_space.shape).prod()
    )  # For jit we need to declare prod outside of class
    critic = Critic()
    ...
    tx = optax.chain(
        ...
        optax.inject_hyperparams(optax.adamw)(learning_rate=linear_schedule, eps=1e-5),
    )
    ...
    try:
        for update in tqdm(range(1, args.num_updates + 1)):
            next_obs, next_done, storage, action_key, global_step = rollout(
                agent_state,
                next_obs,
                next_done,
                storage,
            ...
            (
                ...
                explained_var,
                permutation_key,
            ) = update_ppo(agent_state, storage, permutation_key)
