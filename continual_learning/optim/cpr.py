...
def continuous_reset_weights(
    ...
    for layer_key in hidden_keys:
        ...
        logs[layer_key] = {
            ...
            "low_utility": jnp.sum(utilities[layer_key] < 0.95),
...
def cpr(
    ...
    def update(
        ...
        def _cpr(
            ...
            # Reconstruct params: replace modified kernels/biases in the
            # full flat dict, then unflatten back to the original tree.
            # Uses flatten_dict(params) (not just params["params"]) so that
            # extra collections like activations/preactivations are preserved.
            flat_full = flax.traverse_util.flatten_dict(params)
            for layer_key in _weights:
                flat_full[("params",) + layer_key + ("kernel",)] = _weights[layer_key]
            for layer_key in _biases:
                flat_full[("params",) + layer_key + ("bias",)] = _biases[layer_key]

            sorted_keys = sorted(flat_full.keys())
            new_leaves = [flat_full[k] for k in sorted_keys]

            return (
                jax.tree.unflatten(jax.tree.structure(params), new_leaves),
                new_state,
                tx_state,
            )
