...
def create_chart(
    ...
    if len(df["metric"].unique()) > 1:
        ...
        for metric in df["metric"].unique():
            ...
            axis_config = alt.Axis(labelFontSize=12, titleFontSize=14)
        ...
        metric_name, y_label = resolve_metric_labels(metric)
        legend_domain = build_algorithm_legend_domain(df["algorithm"]) if not df.empty else []
        color_scale = (
            alt.Scale(domain=legend_domain)
            if legend_domain
            else alt.Undefined
    ...
    axis_config = alt.Axis(labelFontSize=14, titleFontSize=16)
...
def main(
    ...
    chart = create_chart(df, metrics, title, combine_networks, show_iqr=show_iqr)
