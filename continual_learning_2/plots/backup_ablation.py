...
def create_short_labels(df: pd.DataFrame, short_labels: bool = True) -> pd.DataFrame:
    ...
    def shorten_group_name(group_name: str) -> str:
        # Skip shortening for simple algorithm names or parameter=value pairs
        if '=' not in group_name:
            return group_name
            
        # Split by underscores and commas to handle different formats
        parts = []
        for part in group_name.split('_'):
            if '=' in part:
                # Handle comma-separated key=value pairs within a part
                subparts = []
                for subpart in part.split(','):
                    if '=' in subpart:
                        key, value = subpart.split('=', 1)
                        # Create initials from key (e.g., "redo_tx_rl" -> "rtr")
                        if '_' in key:
                            initials = ''.join([word[0] for word in key.split('_')])
                        else:
                            initials = key[:3]  # First 3 chars if no underscores
                        subparts.append(f"{initials}={value}")
                    else:
...
def main(
    ...
    # Create chart
    chart = create_ablation_chart(agg_df, metric, title)
