...
def fetch_group_data(entity: str, project: str, group_name: str, 
...
def aggregate_metrics(data: Dict[str, Dict[str, np.ndarray]], 
                     metric: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    iqm_vals = np.array([iqm(stacked[:, i]) for i in range(stacked.shape[1])])
    
    return mean_vals, std_vals, iqm_vals, stacked


def get_best_runs(entity: str, project: str, metric: str, 
                 direction: str = 'max', n: int = 5) -> List[Any]:
    api = get_api()
    order = f"-summary_metrics.{metric}" if direction == 'max' else f"summary_metrics.{metric}"
    return list(api.runs(f"{entity}/{project}", {"$and": [{"summary_metrics": {"$exists": True}}]}, 
                        order=order))[:n]


def compare_groups(entity: str, project: str, groups: List[str], 
                  metric: str) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    return {group: aggregate_metrics(fetch_group_data(entity, project, group, [metric]), metric)[:2] 
            for group in groups}