...
class PPOTrainer(PPO):
    """PPO Trainer for standard Gym environments"""
    ...
    def __init__(
        ...
        obs_spec = single_env.observation_space
        action_spec = single_env.action_space
        single_env.close()
        ...
        if isinstance(action_spec, gym.spaces.Discrete):
            ...
            raise ValueError(f"Unsupported action space: {action_spec}")
            
        if isinstance(obs_spec, gym.spaces.Box):
            obs_shape = obs_spec.shape
        else:
            raise ValueError(f"Unsupported observation space: {obs_spec}")
...
if __name__ == "__main__":
    ...
    start = time.time()
    ant_trainer = PPOTrainer(
        env_id="Ant-v5",
        seed=SEED,
        ppo_config=PPOConfig(
            policy_config=PolicyNetworkConfig(
                optimizer=AdamConfig(learning_rate=2e-4),
                network=MLPConfig(
                    num_layers=3,
                    hidden_size=256,
                ...
                std_type=StdType.MLP_HEAD,  # For continuous control
            ),
            ...
            num_rollout_steps=4096,    # Larger rollouts for continuous control
            num_epochs=8,             # More epochs for stable learning
            num_gradient_steps=32,
            gamma=0.99,                # Standard for continuous control
            gae_lambda=0.95,
            entropy_coefficient=0.001,   # Lower entropy for continuous control
            clip_eps=0.2,
            vf_coefficient=0.5,
        ...
        logs_cfg=LoggingConfig(
            run_name="ant_v5_ppo_experiment",
            wandb_entity="lucmc",
            wandb_project="crl_experiments",
            save=True,  # Save checkpoints for long training runs
            wandb_mode="online",
        ),
        num_envs=8,    # More parallel environments for sample efficiency
        async_envs=True,  # Async for better performance with MuJoCo
    )
    
    print("Training Ant-v5 with PPO...")
    ant_trainer.train()
    # trainer.train()
