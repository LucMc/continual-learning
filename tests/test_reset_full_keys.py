...
def test_cbp_handles_single_prefixed_policy_network_features():
    params = {
        "params": {
            "main": {
                "layer_0": {
    ...
    features = {"main": {"layer_0_act": (jnp.ones((4, 3)),)}}
    ...
    assert ("main", "layer_0") in new_state.utilities
    assert jnp.all(new_state.utilities[("main", "layer_0")] > 0.0)
